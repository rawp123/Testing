...
The message import endpoint maps handles, chats, chat-message joins, message text, and attachment metadata into the normalized archive database. It reads `message.text` first and falls back to readable `attributedBody`/`payload_data` content when `message.text` is empty. It does not copy attachment files or read attachment payload data, and it returns counts only.
