(function initializeAllInPodcastSearch() {
  ...
  const elements = {
    ...
    episodesPanel: document.getElementById('aps-panel-episodes'),
    searchPanel: document.getElementById('aps-panel-search'),
    ...
    inlinePlayerFrame: document.getElementById('aps-inline-player-frame'),
    inlinePlayerFallback: document.getElementById('aps-inline-player-fallback'),
    inlinePlayerStatus: document.getElementById('aps-inline-player-status'),
  ...
  const state = {
    ...
    activeTab: document.body.dataset.apsView === 'search' ? 'search' : 'episodes',
    searchToken: 0,
    query: '',
    selectedEpisodeId: '',
    selectedHitId: '',
  ...
  function getEpisodeGuests(episode) {
    return Array.isArray(episode.guests) ? episode.guests.filter(Boolean) : [];
  }

  function getEpisodeTopics(episode) {
  ...
  function describeSearchCoverage(episode) {
  ...
  function formatDuration(episode) {
    return episode.durationLabel || '—';
  }

  function getEpisodeLabel(episode) {
    const numericEpisode = Number(episode.episodeNumber);
    if (Number.isFinite(numericEpisode)) {
      return String(numericEpisode);
    }

    if (getEpisodeGuests(episode).length) {
      return 'Special';
    }
  ...
  function getFullEpisodeLink(episode) {
  ...
  function setActiveTab(tab, options = {}) {
    state.activeTab = tab === 'search' ? 'search' : 'episodes';

    if (options.focusSearch && hasSearchView) {
  ...
  function describeResultMeta(match) {
    const parts = [shared.formatDate(match.publishDate)];

    if (match.speaker) {
      parts.push(match.speaker);
    } else if (match.guests?.length) {
      parts.push(match.guests.join(', '));
    }
              ...
              ${sortedMatches.map((match) => `
                <tr class="aps-search-result-row ${match.chunkId === state.selectedHitId ? 'is-selected' : ''}" data-hit-id="${shared.escapeHtml(match.chunkId)}" data-episode-id="${shared.escapeHtml(match.episodeId)}" data-row-url="${shared.escapeHtml(match.detailUrl)}" tabindex="0" role="link" aria-label="Open ${shared.escapeHtml(match.episodeTitle)} at ${shared.escapeHtml(match.timestampLabel)}">
                  ...
                  <td data-label="Transcript Snippet">
                    <p class="aps-result-snippet">${highlightText(match.snippetText, state.query)}</p>
