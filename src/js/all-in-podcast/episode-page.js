...
import { getFullEpisodeLink } from './catalog.js';
import { highlightText } from './utils.js';

export function initializeAllInPodcastEpisode() {
  ...
  function renderTranscript() {
    ...
    elements.transcriptList.innerHTML = state.episode.chunks.map((chunk) => `
      <article class="aps-transcript-chunk" data-chunk-id="${shared.escapeHtml(chunk.id)}" id="${shared.escapeHtml(chunk.id)}" tabindex="-1">
        ...
        <h3 class="aps-transcript-title">${state.query ? highlightText(chunk.excerpt, state.query, shared.escapeHtml) : shared.escapeHtml(chunk.excerpt)}</h3>
        <p class="aps-transcript-body">${state.query ? highlightText(chunk.text, state.query, shared.escapeHtml) : shared.escapeHtml(chunk.text)}</p>
      </article>
    `).join('');
  }
  ...
  function renderEpisode() {
    ...
    const fullEpisodeHref = getFullEpisodeLink(episode)?.href || '';
