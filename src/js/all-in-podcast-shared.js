(function attachAllInPodcastShared() {
  const DATA_ROOT = '/data/all-in-podcast-search';
  ...
  function buildMomentUrl(episodeId, startSeconds, chunkId, options = {}) {
  ...
  function buildEpisodeUrl(episodeId, options = {}) {
    const baseUrl = new URL('/all-in-podcast-search/all-in/episode/', window.location.origin);
    baseUrl.searchParams.set('id', episodeId);
  ...
  function findChunkForTime(chunks, totalSeconds) {
  ...
  async function loadEpisodesCatalog() {
